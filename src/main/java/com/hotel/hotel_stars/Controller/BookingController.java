...
//khoi
	...
	public StatusResponseDto postBookingOffline(@Valid @RequestBody bookingModel bookingModels) {
		...
		if (flag == true) {
			return new StatusResponseDto("200", "success", "Đặt phòng thành công");
		} else {
			return new StatusResponseDto("200", "success", "Đặt phòng thất bại");
		}
	}
	...
	public ResponseEntity<?> downloadPdf(@RequestParam String id) {
		try {
			...
			String filePath = paramServices.generatePdf(
					paramServices.pdfDowload(idBk, booking, startDate, endDate, formattedAmount, roomsString),
