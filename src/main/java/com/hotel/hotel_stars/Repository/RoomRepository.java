...
public interface RoomRepository extends JpaRepository<Room, Integer> {
    @Query(value = "SELECT " + "(SELECT COUNT(*) FROM accounts WHERE role_id = 2) AS count_employees, "
            ...
            + "FROM accounts " + "GROUP BY count_employees, count_customers", nativeQuery = true)
    List<Object[]> getCounts();

    @Query(value = "SELECT type_room.id AS type_room_id, " + "type_room.type_room_name, "
            + "COUNT(room.id) AS room_count, " + "type_room.price, " + "type_bed.id AS type_bed, "
            + "type_room.bed_count, " + "type_room.guest_limit, " + "type_room.acreage, " + "type_room_image.id "
            + "FROM room " + "JOIN type_room ON room.type_room_id = type_room.id "
            ...
            + "type_bed.id, type_room.bed_count, type_room.guest_limit, type_room.acreage, type_room_image.id", nativeQuery = true)
    ...
    @Query(value = "SELECT " +
            ...
            "type_room.describes, " +
            "discount.id, " +
            "discount.percent", nativeQuery = true)
    List<Object[]> findRoomDetailsByRoomId(Integer roomId);

    @Query(value = "SELECT " + "room.id AS roomId, " + "type_room.id AS typeRoomId, "
            + "type_room.type_room_name AS typeRoomName, " + "type_room.price, " + "type_room.bed_count AS bedCount, "
            + "type_room.acreage, " + "type_room.guest_limit AS guestLimit, " + "type_bed.bed_name AS bedName, "
            + "type_room.describes, " + "GROUP_CONCAT(DISTINCT type_room_image.id) AS imageNames, "
            + "GROUP_CONCAT(DISTINCT amenities_type_room.amenities_type_room_name) AS amenities, " + "(CASE "
            + "    WHEN discount.id IS NULL OR NOT (NOW() BETWEEN discount.start_date AND discount.end_date) "
            + "    THEN 0 " + "    ELSE (type_room.price * (1 - IFNULL(discount.percent, 0) / 100)) "
            + "END) AS finalPrice, " + "(TIMESTAMPDIFF(DAY, CURDATE(), DATE_ADD(CURDATE(), INTERVAL 1 DAY)) * "
            + "(CASE "
            + "    WHEN discount.id IS NULL OR NOT (NOW() BETWEEN discount.start_date AND discount.end_date) "
            + "    THEN type_room.price " + "    ELSE (type_room.price * (1 - IFNULL(discount.percent, 0) / 100)) "
            + "END)) AS estCost, " + "discount.percent " + "FROM type_room "
            + "JOIN type_room_image ON type_room.id = type_room_image.type_room_id "
            + "JOIN type_bed ON type_room.type_bed_id = type_bed.id "
            + "JOIN type_room_amenities_type_room ON type_room.id = type_room_amenities_type_room.type_room_id "
            + "JOIN amenities_type_room ON type_room_amenities_type_room.amenities_type_room_id = amenities_type_room.id "
            + "JOIN room ON type_room.id = room.type_room_id "
            + "LEFT JOIN discount ON type_room.id = discount.type_room_id " + "WHERE room.id = ?1 "
            ...
            + "type_room.describes, " + "discount.id, " + "discount.percent", nativeQuery = true)
    List<Object[]> findRoomDetailsByRoomIds(Integer roomId);
    ...
    @Query(value = """
                ...
                INNER JOIN TypeRoom tr ON tr.id = r.typeRoom.id
                WHERE
                    (b.id IS NULL OR (:startDate <= DATE(b.endAt) AND :endDate >= DATE(b.startAt)))
                    AND NOT EXISTS (
                        SELECT 1
                        FROM BookingRoom br_inner
                        JOIN Booking b_inner ON br_inner.booking.id = b_inner.id
                        WHERE br_inner.room.id = r.id
                        AND (
                            DATE(b_inner.startAt) <= :endDate
                            AND DATE(b_inner.endAt) >= :startDate
                        )
                    )
                    AND tr.guestLimit >= :guestLimit
                ORDER BY r.roomName
            """)
    Page<Room> findAvailableRoomsWithPagination(
    ...
    @Query(value = """
                ...
                ORDER BY r.roomName
            """)
    Page<Room> findAvailableRoomsWithPagination(@Param("startDate") Date startDate, @Param("endDate") Date endDate,
            @Param("guestLimit") Integer guestLimit, Pageable pageable);
    // khôi
