...
public class RoomService {
	...
	public PaginatedResponseDto<RoomDto> getAll(int page, int size, String sortBy, Date startDates, Date endDates, Integer guestLimit) {
	    Pageable pageable = PageRequest.of(page, size, Sort.by(sortBy));
	    Page<Room> roomPage = roomRepository.findAvailableRoomsWithPagination(startDates, endDates, guestLimit, pageable);
...
//	public PaginatedResponseDto<RoomDto> getAll(int page, int size, String sortBy, Instant startDate, Instant endDate, Integer guestLimit) {
//	    int offset = (page - 1) * size;
//
//	    List<Object[]> roomData = roomRepository.findAvailableRoomsWithPagination(startDate, endDate, guestLimit, size, offset);
//
//	    List<RoomDto> roomDtos = roomData.stream().map(this::convertToDto).toList();
//
//	    long totalElements = roomRepository.countAvailableRooms(startDate, endDate, guestLimit); // Thêm phương thức đếm tổng số phòng
//
